...
def main():
    input_file_path = 'C:/Workshop_Problem/MilestoneInputs/Input/Milestone3a.json'
    output_file_path = 'C:/Workshop_Problem/Milestones_output/Output_3a.json'
    ...
    while str_list:
        for s in str_list[:]:
            ...
            if w[w_need] <= i and machine_available_time[assigned_machine] <= i:
                ...
                # Cooldown
                parameter_key = list(machine_parameters[assigned_machine].keys())[0]
                if not (json_decode['steps'][int(step_id[1:])-1]['parameters'][parameter_key][0] <= machine_parameters[assigned_machine][parameter_key] <= json_decode['steps'][int(step_id[1:])-1]['parameters'][parameter_key][1]):
                    ...
                    machine_parameters[assigned_machine][parameter_key] = json_decode['machines'][int(assigned_machine[1:])-1]['initial_parameters'][parameter_key]
                ...
                if machine_step_count[assigned_machine] >= json_decode['machines'][int(assigned_machine[1:])-1]['n']:
                    machine_parameters[assigned_machine][parameter_key] += json_decode['machines'][int(assigned_machine[1:])-1]['fluctuation'][parameter_key]
